    ...
    def update(self):
        ...
        for epoch_i in range(self.EPOCH):
            ...
            with torch.no_grad():
                ...
                memo_values_tensor = torch.tensor(np.array(memo_value)).to(device)
                
            memo_states_tensor = torch.FloatTensor(np.array(memo_states)).to(device)
            ...
            for batch in batches:
                ...
                mu = torch.clamp(mu, min=-1e3, max=1e4)
                sigma = torch.clamp(sigma, min=1e-4, max=1e3)  # 确保 sigma 为正数
